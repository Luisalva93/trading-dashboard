...
function SmallStat({ label, value, color, tooltip }) {
  return (
    <div title={tooltip || ''} style={{
      ...
      borderRadius: '10px', padding: '12px 14px', cursor: tooltip ? 'help' : 'default',
    }}>
  ...
  return (
    <div style={{ marginBottom: '20px' }}>
      ...
      <div style={{
        ...
        <BigStat label="P&L del Mes" value={`${totalPnl >= 0 ? '+' : ''}$${totalPnl.toFixed(0)}`} color={pnlColor} />
        <BigStat label="Profit Factor" value={profitFactor}
          color={pfVal >= 1.5 ? 'var(--green)' : pfVal >= 1 ? '#f59e0b' : 'var(--red)'}
          subtitle={pfVal >= 1.5 ? 'Bueno' : pfVal >= 1 ? 'Neutral' : 'Bajo'} />
        <BigStat label="Trade Win %" value={`${tradeWinPct}%`}
          color={tradeWinPct >= 55 ? 'var(--green)' : tradeWinPct >= 45 ? '#f59e0b' : 'var(--red)'}
          subtitle={`${tradedDays} días operados`} />
        <BigStat label="Sharpe Ratio" value={sharpe !== null && sharpe !== undefined ? sharpe : '—'}
          color={sharpeColor}
          subtitle={!sharpe ? '' : sharpe >= 1 ? 'Bueno' : sharpe >= 0 ? 'Neutral' : 'Bajo'} />
        <BigStat label="Recovery Factor" value={recovery !== null && recovery !== undefined ? recovery : '—'}
          color={recoveryColor}
          subtitle={!recovery ? '' : recovery >= 2 ? 'Excelente' : recovery >= 1 ? 'Aceptable' : 'Bajo'} />
      </div>
      ...
      <div style={{
        ...
        <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: '10px', padding: '12px 14px' }}>
          ...
          <div style={{ fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 'clamp(0.85rem, 1.8vw, 1rem)' }}>
            ...
            <span style={{ color: 'var(--text-muted)', margin: '0 3px', fontSize: '0.8rem' }}>/</span>
        ...
        {maxDD > 0 && <SmallStat label="Max Drawdown" value={`-$${parseFloat(maxDD).toFixed(0)}`} color="var(--red)" tooltip="Caída máxima desde un pico hasta un valle" />}
      </div>
    </div>
  );
}
