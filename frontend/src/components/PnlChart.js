...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: '10px', padding: '12px 16px', boxShadow: '0 4px 24px rgba(0,0,0,0.4)' }}>
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '16px', padding: '20px 20px 14px', marginBottom: '24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        ...
        <div style={{ fontFamily: 'var(--mono)', fontWeight: 700, fontSize: '1.1rem', color: isPositive ? 'var(--green)' : 'var(--red)' }}>
          {isPositive ? '+' : ''}${cumulative.toFixed(0)}
        </div>
      </div>
      <ResponsiveContainer width="100%" height={180}>
        <AreaChart data={data} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
          ...
          <Area type="monotone" dataKey="acumulado" stroke={strokeColor} strokeWidth={2.5}
            fill="url(#chartGrad)" dot={false} activeDot={{ r: 5, fill: strokeColor, strokeWidth: 0 }} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
