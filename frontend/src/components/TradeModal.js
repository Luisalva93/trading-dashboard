...
function TradeRow({ trade, onUpdate, onDelete }) {
  ...
  if (editing) return (
    <div style={{ background: 'var(--surface2)', borderRadius: '10px', padding: '12px', marginBottom: '8px', border: '1px solid var(--accent)' }}>
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        <button onClick={() => onDelete(trade.id)} style={{ padding: '8px 12px', borderRadius: '8px', border: '1px solid var(--red-border)', background: 'transparent', color: 'var(--red)', cursor: 'pointer', fontSize: '0.8rem' }}>Eliminar</button>
        <button onClick={() => setEditing(false)} style={{ flex: 1, padding: '8px', borderRadius: '8px', border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-muted)', cursor: 'pointer' }}>Cancelar</button>
        <button onClick={() => { onUpdate(trade.id, { pnl: parseFloat(pnl), instrument, setup, entry_time: entryTime }); setEditing(false); }} style={{ flex: 2, padding: '8px', borderRadius: '8px', border: 'none', background: 'var(--accent)', color: '#fff', cursor: 'pointer', fontWeight: 600 }}>Guardar</button>
      </div>
    </div>
  );
  ...
  return (
    <div onClick={() => setEditing(true)}
      style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--surface2)', borderRadius: '10px', padding: '12px 14px', marginBottom: '8px', cursor: 'pointer', border: `1px solid ${pnlVal > 0 ? 'var(--green-border)' : pnlVal < 0 ? 'var(--red-border)' : 'var(--border)'}` }}>
...
export default function TradeModal({ date, session, onClose, onRefresh }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setDayData(data); setNotes(data.notes || '');
    } catch {}
    setLoading(false);
  };
  ...
  const handleAddTrade = async () => {
    if (newPnl === '') return;
    await addTrade({ date, pnl: parseFloat(newPnl), instrument: newInstrument, setup: newSetup, entry_time: newEntryTime, session });
  ...
  return (
    <div onClick={e => e.target === e.currentTarget && onClose()}
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '20px 20px 0 0', padding: '20px 20px 32px', width: '100%', maxWidth: '500px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 -4px 40px rgba(0,0,0,0.5)' }}>
        ...
        {loading ? <div style={{ color: 'var(--text-muted)', padding: '20px 0', textAlign: 'center' }}>Cargando...</div> : (
          <>
            ...
            {showAddForm ? (
              <div style={{ background: 'var(--surface2)', borderRadius: '12px', padding: '14px', marginBottom: '12px', border: '1px solid var(--accent)' }}>
                <div style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text)', marginBottom: '12px' }}>Nuevo trade — {sessionLabel}</div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button onClick={() => setShowAddForm(false)} style={{ flex: 1, padding: '11px', borderRadius: '8px', border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-muted)', cursor: 'pointer' }}>Cancelar</button>
                  <button onClick={handleAddTrade} style={{ flex: 2, padding: '11px', borderRadius: '8px', border: 'none', background: 'var(--accent)', color: '#fff', cursor: 'pointer', fontWeight: 600 }}>Agregar Trade</button>
                </div>
              </div>
            ) : (
              <button onClick={() => setShowAddForm(true)} style={{ width: '100%', padding: '13px', borderRadius: '10px', border: '1px dashed var(--border)', background: 'transparent', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '0.9rem', marginBottom: '12px' }}>
                + Agregar trade
              </button>
            )}
            ...
            <div style={{ marginTop: '4px' }}>
              ...
              <textarea placeholder="¿Cómo fue la sesión?" value={notes} onChange={e => setNotes(e.target.value)}
                onBlur={() => saveNotes(date, notes, session)}
                style={{ ...inputStyle, fontFamily: 'var(--sans)', fontSize: '0.9rem', resize: 'none', minHeight: '80px', lineHeight: 1.5 }} />
              <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '4px' }}>Se guarda automáticamente al salir del campo</div>
            </div>

            <button onClick={onClose} style={{ width: '100%', padding: '13px', borderRadius: '10px', marginTop: '16px', border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-muted)', cursor: 'pointer' }}>Cerrar</button>
          </>
        )}
      </div>
    </div>
